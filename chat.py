...
def get_prompt_deepseek(instruction: str, history: list[str] = None) -> str:
    system = 'You are an AI assistant that gives helpful answers. You answer the question in a short and concise way.'
    prompt = f"<s>[INST] <<SYS>>\n{system}\n<</SYS>>\n\n"
    if history is not None:
        prompt += f"This is the conversation history: {''.join(history)}. Now answer the question: "
...
def select_llm(llm_name: str):
    ...
    if llm_name == 'llama2':
        ...
        get_prompt = get_prompt_deepseek
        return 'Model changed to Deepseek'
    else:
...
async def on_message(message: cl.Message):
    if message.content.lower() in ['use llama2', 'use orca', 'use deepseek']:
...
async def on_chat_start():
    await cl.Message('Loading WadiGPT model...').send()
    select_llm('orca')
    cl.user_session.set('message_history', [])

    await cl.Message(
        'Hi 👋🏾, my name is WadiGPT. How can I be of service to you?'
    ).send()
